...
| configure probes | jig makes each probe compile a hit, but the shell around hundreds of them still costs 20–60 s. autotools.nu passes `--cache-file` and keeps that file in the build cache (`jig cache`), keyed on sha256(configure) + hash-masked toolchain/dependency set + platform + flags + `$out`; autoconf additionally refuses a cache whose precious vars differ |
| upstream binaries (`prebuilt`) | not patched: launch runs them as `<sysroot>/lib/ld.so --argv0 bin/foo --library-path <libc:deps' libDirs> bin/.foo`, `libgcc-shim` supplies `libgcc_s.so.1` over libunwind. Used for rustc/cargo (docs/plan.md 5) |
| shebangs, env wrappers | one static-pie `launch` binary in its own store path. `bin/foo` is a symlink `../../<hash>-launch/bin/launch`, the record sits beside it as `bin/.foo.launch` (JSON: `program`, `args`, optional `argv0`, `env` with `{root}`/`{store}`/`{self}` templates), the real script or ELF as `bin/.foo`. argv[0] defaults to the program so interpreters find their prefix. Replaces `#!` lines and `makeWrapper`. fixup converts every script and every ELF that has runtimeDependencies. |
