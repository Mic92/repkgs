# purl type → list of {version date prerelease tag? url? sha256?}. All GETs go through the cache,
# so the second run over a tree is 304s.

use version.nu *
use http.nu *

# all versions the purl's datasource lists
export def versions [p: record]: nothing -> table {
  match $p.type {
    github => (github $p)
    pypi => (pypi $p)
    cargo => (crates $p)
  ...
  } | default false prerelease | update prerelease {|r| $r.prerelease or (version is-prerelease $r.version) } | where version =~ '^\d'
}
...
def github [p: record]: nothing -> table {
  ...
  fetch $"($repo)/tags?per_page=100" $repo | each {|t| {version: (version from-tag $t.name), date: null, tag: $t.name} }
}

def pypi [p: record]: nothing -> table {
...
def crates [p: record]: nothing -> table {
...
def npm [p: record]: nothing -> table {
...
def listing [url: string, name: string, --regex: oneof<string, nothing>]: nothing -> table {
  let re = if $regex != null { $regex } else { '(?:^|[/">])' + $name + '-(\d[0-9a-z.]*?)\.(?:tar\.(?:xz|lz|bz2|gz|zst)|zip|tgz)' }
...
def generic [p: record]: nothing -> table {
