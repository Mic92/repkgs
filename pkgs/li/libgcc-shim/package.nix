# lib/libgcc_s.so.1 with the GCC_* symbol versions upstream-built binaries link, backed by our
# libunwind. Only `prebuilt` packages need it, our libc loads libunwind.so.1 directly.
{ package, toolchain }:
package {
  name = "libgcc-shim";
  version = "1";
