...
# patches (store paths, so exact) plus what build-cache.nu keys on: dependencies, flags
use core.nu *
use build-cache.nu

def key []: nothing -> string {
  let a = (attrs)
  build-cache key $"zig-cache/($a.src | path basename)" $a.patches
}

export def restore []: nothing -> nothing {
  note zig-cache (if (build-cache restore-dir (key) $env.ZIG_GLOBAL_CACHE_DIR) { "restored" } else { "cold" })
}

export def store []: nothing -> nothing { build-cache store-dir (key) $env.ZIG_GLOBAL_CACHE_DIR }
