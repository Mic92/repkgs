...
# ./configure --prefix=$out, Autoconf defaults, plus `autotools.flags`.
export def --env configure []: nothing -> nothing {
  let c = (ctx); let o = (options autotools)
  let script = $"($c.src)/($o.configure.script)"
  ...
  let autoconf_flags = (if $autoconf {
    ...
    [$"--cache-file=($cache)" --disable-nls --disable-dependency-tracking $"INSTALL=(install-tool) -c" --disable-static --enable-shared]
      | append $host_flags
  } else { [] })
  with-env (if $o.configure.parallel { {MAKEFLAGS: $"-j($c.njobs)"} } else { { } }) {
    x $env.CONFIG_SHELL $script $"--prefix=($c.out)" ...$autoconf_flags ...($o.flags)
  }
