...
def main []: nothing -> nothing {
  ...
  let vsix = ([Headers $"($crt_arch).Desktop" $"($crt_arch).Store" $"Redist.($crt_arch | str uppercase)"] | each {|k| do $package $"Microsoft.VC.($crt).CRT.($k).base" | get payloads.0 | insert kind crt })
